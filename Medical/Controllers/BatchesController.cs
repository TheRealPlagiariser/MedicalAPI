using System;
using System.Collections.Generic;
using System.Data;
using System.Data.Entity;
using System.Data.Entity.Infrastructure;
using System.Linq;
using System.Net;
using System.Net.Http;
using System.Web.Http;
using System.Web.Http.Description;
using System.Threading.Tasks;
using Medical.Models;

namespace Medical.Controllers
{
    public class BatchesController : ApiController
    {
        private MedicalContext db = new MedicalContext();

        // GET: api/Batches
        //public IQueryable<Batch> GetBatches()
        //{
        //    return db.Batches;
        //}

        // GET: api/Batches/5
        [ResponseType(typeof(Batch))]
        public IHttpActionResult GetBatch()
        {
            var query = from b in db.Batches
                        where b.batch_date_to == null
                        select new
                        {
                            b.batch_id
                        };
            //Batch batch = db.Batches.Find(id);
            //if (batch == null)
            //{
            //    return NotFound();
            //}

            return Ok(query);
        }

        [ResponseType(typeof(Batch))]
        [HttpPost]
        public async Task<HttpResponseMessage> UpdateBatchdate(Batch batch)
        {
            
            Batch slot = await db.Batches.FirstOrDefaultAsync(x => x.batch_date_to == null );

            slot.batch_date_to= DateTime.Now;

           db.Entry(slot).State = EntityState.Modified;
            await db.SaveChangesAsync();

            Batch temp = new Batch();
            temp.batch_date_from = DateTime.Now;
            db.Batches.Add(temp);
            await db.SaveChangesAsync();

            return Request.CreateResponse(HttpStatusCode.OK, "Update successful");
        }

        //// PUT: api/Batches/5
        //[ResponseType(typeof(void))]
        //public IHttpActionResult PutBatch(int id, Batch batch)
        //{
        //    if (!ModelState.IsValid)
        //    {
        //        return BadRequest(ModelState);
        //    }

        //    if (id != batch.batch_id)
        //    {
        //        return BadRequest();
        //    }

        //    db.Entry(batch).State = EntityState.Modified;

        //    try
        //    {
        //        db.SaveChanges();
        //    }
        //    catch (DbUpdateConcurrencyException)
        //    {
        //        if (!BatchExists(id))
        //        {
        //            return NotFound();
        //        }
        //        else
        //        {
        //            throw;
        //        }
        //    }

        //    return StatusCode(HttpStatusCode.NoContent);
        //}

        //// POST: api/Batches
        //[ResponseType(typeof(Batch))]
        //public IHttpActionResult PostBatch(Batch batch)
        //{
        //    if (!ModelState.IsValid)
        //    {
        //        return BadRequest(ModelState);
        //    }

        //    db.Batches.Add(batch);
        //    db.SaveChanges();

        //    return CreatedAtRoute("DefaultApi", new { id = batch.batch_id }, batch);
        //}

        // DELETE: api/Batches/5
        [ResponseType(typeof(Batch))]
        public IHttpActionResult DeleteBatch(int id)
        {
            Batch batch = db.Batches.Find(id);
            if (batch == null)
            {
                return NotFound();
            }

            db.Batches.Remove(batch);
            db.SaveChanges();

            return Ok(batch);
        }

        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                db.Dispose();
            }
            base.Dispose(disposing);
        }

        private bool BatchExists(int id)
        {
            return db.Batches.Count(e => e.batch_id == id) > 0;
        }
    }
}